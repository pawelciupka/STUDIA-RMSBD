...
is
id_produktu product.product_id%type := 0;
...
declare
    productName product.name%type := 'Ser gouda 1kg';
    opakowanie package.name%type := 'trojpak plastikowy';
begin 
    add_Package_Of_Product(productName,
                            opakowanie,
                            3, 
...
dbms_output.put_line('znaleziono produktu w magazynie: ' || nazwa_polki);
return nazwa_polki;
end;
/
